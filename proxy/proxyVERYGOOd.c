...
open_client(char *hostname, int port, char *uri)
{
	...
	FILE *fp;
	char *logger = malloc(MAXLINE * sizeof(char));
	...
	format_log_entry(logger, &serveraddr,uri, sizeof(uri));
	fp = fopen("proxy.log", "a");
	printf("**************************This is what is in logger+++++++++%s\n", logger);
	fwrite(logger, sizeof(char), MAXLINE/sizeof(char), fp);
	fclose(fp);
	free(logger);
	freeaddrinfo(ai);
	return clientfd;
...
void doit(int fd) 
{
	...
	clientfd = open_client(&hostname[0], port, uri);
	if (clientfd < 0) {
		Close(clientfd);
		return;
	}
